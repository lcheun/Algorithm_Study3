...
for i in range(n):
    endurance[i], weight[i] = map(int, sys.stdin.readline().split())

def backtracking(index, eggs):
    global ans
    if index == n:
        cnt = 0
        for i in range(n):
    ...
    if eggs[index] > 0:
        ...
        if not flag:
            backtracking(index + 1, eggs)
    else:
        backtracking(index + 1, eggs)
    
    
backtracking(0, endurance)
print(ans)
